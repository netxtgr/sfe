...
package io.sihuan.updaterecord.core.support;

import io.sihuan.updaterecord.annotation.UpdateRecordColumn;
import io.sihuan.updaterecord.annotation.UpdateRecordIgnore;
import io.sihuan.updaterecord.api.interceptor.UpdateRecordInterceptor;
import io.sihuan.updaterecord.api.vo.TableVo;
import io.sihuan.updaterecord.core.cache.UpdateRecordThreadLocalCache;
import io.sihuan.updaterecord.core.compare.UpdateRecordComparator;
...
import io.sihuan.updaterecord.core.exception.UpdateRecordException;
import io.sihuan.updaterecord.core.util.PluginUtil;
...
public abstract class UpdateRecordInterceptorSupport implements UpdateRecordInterceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        try {
            ...
            if (paramObject instanceof Map) {
                ...
                paramObject = ((Map<?, ?>) paramObject).get("et");
                //return invocation.proceed();
            }
    ...
    private void handle(Invocation invocation, String methodId, Object paramObject, List<ParameterMapping> parameterMappings, Class<?> cls, TableVo tableVo, Object idValueObject) throws Exception {
        // 修改项
        UpdateRecordTableLogEntity updateRecordTableLog = new UpdateRecordTableLogEntity();
        // 设置当前会话唯一标识
    ...
    protected Object getResultSetColumnValue(ResultSet resultSet, int i, int columnType) throws SQLException {
        ...
        if (Types.DATE == columnType) {
            ...
            value = date == null ? null : new Date(date.getTime());
        } else if (Types.TIME == columnType || Types.TIME_WITH_TIMEZONE == columnType) {
            Time time = resultSet.getTime(i);
            value = time == null ? null : new Date(time.getTime());
